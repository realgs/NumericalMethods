...
def download_data(date, apikeys=None, peroid='1HRS'):
    ...
    url = 'https://rest.coinapi.io/v1/ohlcv/BITSTAMP_SPOT_BTC_USD/history?period_id={}&time_start={}&limit=100000'
    headers = {'X-CoinAPI-Key': apikeys[0]}
    response = rq.get(url.format(peroid, date), headers=headers)
    if response.status_code != 200:
        apikeys = select_apikey(apikeys)
        return download_data(date, apikeys)
    df = pd.DataFrame(response.json())
    df['time_period_start'] = pd.to_datetime(df['time_period_start'].str.slice(stop=16))
    df['time_period_end'] = pd.to_datetime(df['time_period_end'].str.slice(stop=16))
